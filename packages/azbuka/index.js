...
export default function Azbuka(options) {
  ...
  return {
    async parseDirectory({ dir, output = null, watch = false }) {
      if (!dir) {
        throw new Error('azbuka: parseDirectory requires "dir" as an argument.');
      }
      try {
    ...
    async parseFile({ file, output = null, watch = false }) {
      if (!file) {
        throw new Error('azbuka: parseFile requires "file" as an argument.');
      }
